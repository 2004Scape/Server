export default class CollisionFlagMap {
    ...
    static tileIndex(x: number, z: number) {
        return (x & 0x7) | ((z & 0x7) << 3);
    }

    static zoneIndex(x: number, z: number, level: number) {
    ...
    get(absoluteX: number, absoluteZ: number, level: number) {
        let zoneIndex = CollisionFlagMap.zoneIndex(absoluteX, absoluteZ, level);
    ...
    set(absoluteX: number, absoluteZ: number, level: number, mask: number) {
        let zoneIndex = CollisionFlagMap.zoneIndex(absoluteX, absoluteZ, level);
        let tileIndex = CollisionFlagMap.tileIndex(absoluteX, absoluteZ);
    ...
    add(absoluteX: number, absoluteZ: number, level: number, mask: number) {
        let zoneIndex = CollisionFlagMap.zoneIndex(absoluteX, absoluteZ, level);
        let tileIndex = CollisionFlagMap.tileIndex(absoluteX, absoluteZ);
    ...
    remove(absoluteX: number, absoluteZ: number, level: number, mask: number) {
        let zoneIndex = CollisionFlagMap.zoneIndex(absoluteX, absoluteZ, level);
        let tileIndex = CollisionFlagMap.tileIndex(absoluteX, absoluteZ);
    ...
    allocateIfAbsent(absoluteX: number, absoluteZ: number, level: number) {
        let zoneIndex = CollisionFlagMap.zoneIndex(absoluteX, absoluteZ, level);
    ...
    deallocateIfPresent(absoluteX: number, absoluteZ: number, level: number) {
        let zoneIndex = CollisionFlagMap.zoneIndex(absoluteX, absoluteZ, level);
        this.flags[zoneIndex] = null;
    }

    isZoneAllocated(absoluteX: number, absoluteZ: number, level: number) {
        let zoneIndex = CollisionFlagMap.zoneIndex(absoluteX, absoluteZ, level);
