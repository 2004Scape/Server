...
export default class PathFinder {
    ...
    findPath(
        ...
        if (!pathFound) {
            ...
            const foundApproachPoint: boolean = this.findClosestApproachPoint(
                localDestX,
                localDestZ,
    ...
    private findPath1(
        ...
        while (this.bufWriterIndex != this.bufReaderIndex) {
            ...
            const reached: boolean = ReachStrategy.reached(
                this.flags,
                level,
    ...
    private findPathN(
        ...
        while (this.bufWriterIndex != this.bufReaderIndex) {
            ...
            if (this.currLocalX < relativeSearchSize && this.currLocalZ > 0 && this.directions[this.localIndex(x, z)] == 0 &&
                ...
                let blocked: boolean = false;
                for (let index: number = 1; index < srcSize; index++) {
                    if (!collision.canMove(this.collisionFlag(baseX, baseZ, this.currLocalX + srcSize, this.currLocalZ + index - 1, level), clipFlag1) || !collision.canMove(this.collisionFlag(baseX, baseZ, this.currLocalX + index, z, level), clipFlag2)) {
                        blocked = true;
                        break;
                    }
                }
            ...
            if (this.currLocalX > 0 && this.currLocalZ < relativeSearchSize && this.directions[this.localIndex(x, z)] == 0 &&
                ...
                let blocked: boolean = false;
                for (let index: number = 1; index < srcSize; index++) {
                    if (!collision.canMove(this.collisionFlag(baseX, baseZ, x, this.currLocalZ + index, level), clipFlag1) || !collision.canMove(this.collisionFlag(baseX, baseZ, this.currLocalX + index - 1, this.currLocalZ + srcSize, level), clipFlag2)) {
                        blocked = true;
                        break;
                    }
                }
            ...
            if (this.currLocalX < relativeSearchSize && this.currLocalZ < relativeSearchSize && this.directions[this.localIndex(x, z)] == 0 &&
                ...
                let blocked: boolean = false;
                for (let index: number = 1; index < srcSize; index++) {
                    if (!collision.canMove(this.collisionFlag(baseX, baseZ, this.currLocalX + index, this.currLocalZ + srcSize, level), clipFlag1) || !collision.canMove(this.collisionFlag(baseX, baseZ, this.currLocalX + srcSize, this.currLocalZ + index, level), clipFlag2)) {
                        blocked = true;
                        break;
                    }
                }
