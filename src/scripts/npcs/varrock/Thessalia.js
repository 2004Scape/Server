...
class ThessaliaTalk extends BaseScript {
    *run(player) {
        yield this.chatnpc('chatcon1', `Do you want to buy any fine clothes?`);
        yield this.p_choice(`What have you got?`, `No, thank you.`);

        if (this.choice == 1) {
            yield this.chatplayer('chatcon1', `What have you got?`);
            yield this.chatnpc('chatcon1', `Well, I have a number of fine pieces of clothing on sale`, `or, if you prefer, I can offer you an exclusive, total-`, `clothing makeover?`);

            yield this.p_choice(`Tell me more about this makeover.`, `I'd just like to buy some clothes.`, `No, thank you.`);

            if (this.choice == 1) {
                yield this.chatplayer('chatcon1', `Tell me more about this makeover.`);
                yield this.chatnpc('chatcon1', `Certainly!`);
                yield this.chatnpc('chatcon1', `Here at Thessalia's fine clothing boutique, we offer a`, `unique service where we will totally revamp your outfit`, `to your choosing.`);
                // yield this.chatnpc('chatcon1', `It's on the house, completely free! Tired of always`, `wearing the same old outfit, day in, day out? This is`, `the service for you!`); // OSRS
                yield this.chatnpc('chatcon1', `So what do you say? Interested? We can change either`, `your top or your legwear!`);
                ...
                if (this.choice == 1 || this.choice == 2) {
                    yield this.chatnpc('chatcon1', `Just select what style and colour you would like from`, `this catalogue.`);
                    ...
                    yield this.chatplayer('chatcon1', `No, thank you.`);
                    yield this.chatnpc('chatcon1', `Well, please return if you change your mind.`);
                }
            } else if (this.choice == 2) {
                this.if_close();
