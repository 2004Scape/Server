...
export class ScriptInputNumberNotNullValidator implements ScriptInputValidator<number> {
    ...
    throwMessage(): string {
        return 'An input number was null(-1).';
    }
}
...
export class ScriptInputLocTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < LocType.count;
    }

    throwMessage(): string {
        return 'An input for a Loc type was not in a valid range which it is impossible to spawn a null Loc noob!';
    }
}
...
export class ScriptInputLocAngleValidator implements ScriptInputValidator<number> {
    ...
    throwMessage(): string {
        return 'An input for a Loc angle was out of range. Range should be: 0 to 3.';
    }
}
...
export class ScriptInputLocShapeValidator implements ScriptInputValidator<number> {
    ...
    throwMessage(): string {
        return 'An input for a Loc shape was out of range. Range should be: 0 to 31.';
    }
}
...
export class ScriptInputDurationValidator implements ScriptInputValidator<number> {
    ...
    throwMessage(): string {
        return 'An input duration was out of range. Range should be greater than 0.';
    }
}

export class ScriptInputCoordValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input <= 0x3ffffffffff;
    }

    throwMessage(): string {
        return 'An input coord was out of range. Range should be: 0 to 4398046511103';
    }
}

export class ScriptInputParamTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < ParamType.count;
    }

    throwMessage(): string {
        return 'An input for a Param type was not in a valid range.';
    }
}

export class ScriptInputNpcTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < NpcType.count;
    }

    throwMessage(): string {
        return 'An input for a Npc type was not in a valid range.';
    }
}
...
export class ScriptInputNpcStatValidator implements ScriptInputValidator<number> {
    ...
    throwMessage(): string {
        return 'An input for a Npc stat was not in a valid range. Range should be: 0 to 5';
    }
}
...
export class ScriptInputQueueValidator implements ScriptInputValidator<number> {
    ...
    throwMessage(): string {
        return 'An input for an ai_queue was not in a valid range. Range should be: 0 to 19';
    }
}

export class ScriptInputHuntTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < HuntType.count;
    }

    throwMessage(): string {
        return 'An input for a Hunt type was not in a valid range.';
    }
}

export class ScriptInputNpcModeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= -1 && input <= NpcMode.APNPC5;
    }

    throwMessage(): string {
        return `An input for a Npc mode was not in a valid range. Range should be -1 to ${NpcMode.APNPC5}.`;
    }
}
...
export class ScriptInputHitTypeValidator implements ScriptInputValidator<number> {
    ...
    throwMessage(): string {
        return 'An input for a hit type was not in a valid range. Range should be 0 to 2.';
    }
}

export class ScriptInputSpotAnimTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < SpotanimType.count;
    }

    throwMessage(): string {
        return 'An input for a SpotAnim type was not in a valid range.';
    }
}

export class ScriptInputEnumTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < EnumType.count;
    }

    throwMessage(): string {
        return 'An input for an Enum type was not in a valid range.';
    }
}

export class ScriptInputObjTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < ObjType.count;
    }

    throwMessage(): string {
        return 'An input for an Obj type was not in a valid range.';
    }
}

export class ScriptInputObjCountValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input > 0 && input <= Inventory.STACK_LIMIT;
    }

    throwMessage(): string {
        return `An input for an Obj count was not in a valid range. Range should be: 1 to ${Inventory.STACK_LIMIT}.`;
    }
}

export class ScriptInputObjNotDummyValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return ObjType.get(input).dummyitem === 0;
    }

    throwMessage(): string {
        return 'An input for an Obj was a graphic_only dummyitem.';
    }
}

export class ScriptInputInvTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < InvType.count;
    }

    throwMessage(): string {
        return 'An input for an Inv type was not in a valid range.';
    }
}

export class ScriptInputCategoryTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < CategoryType.count;
    }

    throwMessage(): string {
        return 'An input for an Category type was not in a valid range.';
    }
}

export class ScriptInputIDKTypeValidator implements ScriptInputValidator<number> {
    condition(input: number): boolean {
        return input >= 0 && input < IdkType.count;
    }

    throwMessage(): string {
        return 'An input for an IDK type was not in a valid range.';
    }
}
