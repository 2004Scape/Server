...
// I use these two variables to compare outputs from official and repacked data
let inDir = 'data/pack/client/official';
let outDir = 'data/src';

let media = Jagfile.load(`${inDir}/media`);

fs.mkdirSync(`${outDir}/sprites/meta`, { recursive: true });
...
for (let i = 0; i < media.fileCount; i++) {
    ...
    if (count === 1) {
        ...
        await pix.img.writeAsync(`${outDir}/sprites/${safeName}.png`);

        // ----

        let meta = `${pix.cropX},${pix.cropY},${pix.width},${pix.height},${pix.pixelOrder ? 'row' : 'column'}\n`;
        fs.writeFileSync(`${outDir}/sprites/meta/${safeName}.opt`, meta);
        ...
        await pal.writeAsync(`${outDir}/sprites/meta/${safeName}.pal.png`);
    } else {
        // sprite sheet!
        let sprites = [];
        ...
        await sheet.writeAsync(`${outDir}/sprites/${safeName}.png`);
