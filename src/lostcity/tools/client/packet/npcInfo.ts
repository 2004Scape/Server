...
fs.readdirSync('dump').sort((a, b) => parseInt(a.slice(0, a.length - '.npc.bin'.length)) - parseInt(b.slice(0, b.length - '.npc.bin'.length))).filter(f => f.endsWith('.npc.bin')).forEach(f => {
