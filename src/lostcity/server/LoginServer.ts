    ...
    start() {
        this.server.on('connection', socket => {
            ...
            socket.on('data', async buf => {
                ...
                stream.waiting = 0;
                const data = new Packet2(new Uint8Array(3));
                ...
                if (opcode === 1) {
                    ...
                    if (!account || !(await bcrypt.compare(password.toLowerCase(), account.password))) {
                        ...
                        const reply = new Packet2(new Uint8Array(1));
                        reply.p1(5);
                    ...
                    const reply = new Packet2(new Uint8Array(save.length + 2 + 1));
                    reply.p1(1);
                    ...
                    const reply = new Packet2(new Uint8Array(2));
...
export class LoginClient {
    ...
    async load(username37: bigint, password: string, uid: number): Promise<{ reply: number; data: Packet2 | null }> {
        await this.connect();
        ...
        const request = new Packet2(new Uint8Array(2 + 8 + password.length + 1 + 4));
        ...
        const length = data.g2();
        const data2 = new Packet2(new Uint8Array(length));
        await this.stream.readBytes(this.socket, data2, 0, length);
        const out = new Packet2(new Uint8Array(2 + length));
    ...
    async save(username37: bigint, save: Packet2) {
        await this.connect();
        ...
        const request = new Packet2(new Uint8Array(2 + 8 + 2 + save.data.length));
        request.p2(Environment.WORLD_ID as number);
        request.p8(username37);
